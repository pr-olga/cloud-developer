...
describe('concat', () => {
  it('should concat olga and udacity', () => {
    const result = concat('olga','udacity');
    expect(result).to.equal('olga udacity');
  });

  it('should throw an error since one param is empty', () => {
    expect(()=>{ concat('','udacity') }).to.throw('one param is empty')
  });
